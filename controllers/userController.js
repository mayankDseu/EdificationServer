...
export const  register = catchAsyncError(async(req,res,next)=>{
    ...
    const file=req.file;
     if (!name || !email || !password || !file) return next(new ErrorHandler("Please eneter all fields",400))
...
export const  login = catchAsyncError(async(req,res,next)=>{
    const {email,password}= req.body;
     ...
     if ( !email || !password) return next(new ErrorHandler("Please eneter all fields",400))


     const  user = await User.findOne({email}).select("+password");
     if (!user) return next(new ErrorHandler("User Dosent Exist ",401));
...
export const  logout = catchAsyncError(async(req,res,next)=>{
    ...
    expires:new Date (Date.now()),
        ...
        success:true,
        message:"Loged Out Successfullly"
    })
})
...
export const  updateProfile = catchAsyncError(async(req,res,next)=>{
 ...
 export const updateProfilepicture = catchAsyncError(async(req,res,nex)=>{
    ...
    res.status(200).json({
        success:true,
        message:"Profiel Picture Updated Successfully"
    })
 })
...
await sendEmail(user.email,"Edification Reset Password",message)
   res.status(200).json({
        success:true,
